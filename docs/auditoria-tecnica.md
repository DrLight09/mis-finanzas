...
**Estado:** la CSP ya existe (ver CHANGELOG), pero mantiene `'unsafe-inline'` en `script-src` porque la app todavía tiene **233 atributos `onclick="..."` inline** (eran 247, luego 240 tras Spotify, ver avance abajo). Sin migrarlos todos, no se puede endurecer la CSP de verdad — `'unsafe-inline'` anula buena parte de su protección.

**Avance real:** se construyó el sistema reusable (`js/core/events.js`, ver `CHANGELOG.md#infraestructura--seguridad`) y se migraron **dos módulos completos: Spotify** (7 `onclick` → `data-action`) **y Mesada** (7 `onclick` → `data-action`: 5 generados dinámicamente en template strings + 2 estáticos que quedaban sueltos en el HTML fijo del sheet, fuera del alcance del barrido original) — 0 restantes en ambos módulos. Mesada mueve plata, a diferencia del criterio de priorización sugerido más abajo ("empezar por los que no mueven plata") — se migró en esta sesión porque así se pidió, no por cambio de criterio; la recomendación de dejar los módulos con plata para después de agotar los que no la mueven sigue en pie para lo que queda. Se sigue confirmando lo mismo que ya se había verificado: el resto de los `onclick` pasan ids/índices/valores de listas fijas como argumento, no texto libre de usuario — **no hay hueco de seguridad abierto por esto**, es trabajo de arquitectura pendiente, no un riesgo activo.
...
**Avance real:** al migrar Spotify aparecieron 3 casos más que ese barrido no había agarrado, porque el valor pasaba por una función (`spNombreDe(x)`) y no por el nombre de campo directo — dos en `.innerHTML` (nombre en la fila del integrante, nombre en un atributo `title`) y varios en `toast()`. Corregidos (ver `CHANGELOG.md#infraestructura--seguridad`). Al migrar Mesada se repitió exactamente el mismo patrón, con otra función distinta: `fuenteLabel()` devuelve el nombre de una cajita/cuenta personalizada (texto libre) sin escapar — 3 casos más corregidos en `abrirDetalleMesada()`. Confirma dos veces seguidas el riesgo que ya describía este punto: el barrido por nombre de campo conocido no detecta texto libre que llega envuelto en una función, y no parece ser un caso aislado de un solo módulo.
...
1. **Migrar los 233 `onclick` restantes → `data-action`/`Events`**, de a un módulo por vez sobre el sistema ya construido (`js/core/events.js`, probado en Spotify y Mesada), priorizando los que no mueven plata (habilita quitar `'unsafe-inline'` de la CSP).
