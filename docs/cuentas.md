...
- **"Mover a otra cuenta" solo se puede usar con saldo ≥ $1,00 en esa cuenta.** El botón queda deshabilitado (`actualizarBotonesTransferir()`) y el origen de Transferir nunca lista cuentas vacías; el **destino** sí puede estar vacío, porque la plata entra. La regla de "qué cuentas se ofrecen cuando la plata sale" vive en `js/core/fuentes-filtro.js` (`FuentesFiltro`), no se repite en cada sheet.
