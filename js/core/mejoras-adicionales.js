...
//
// Fusión de mejoras.js + mejoras-adicionales.js (2026-09-10): ambos eran
// literalmente la parte 1 y la parte 2 de un mismo bloque original llamado
// "MEJORAS ADICIONALES" (ver el propio encabezado que tenía cada uno antes
// de esta fusión) — no una separación real de responsabilidad, sino el
// corte arbitrario de la sesión de extracción original. Mismo criterio que
// ya se usó para fusionar sheet-swipe.js + sheet-viewport.js en
// sheet-behavior.js: dos archivos con nombres casi idénticos (uno de ellos
// literalmente "mejoras-adicionales", el otro apodado igual en su propio
// comentario) invitaban a confundirlos o a no saber en cuál de los dos
// buscar algo. Ver CHANGELOG.md para el detalle.
//
// No hay ninguna dependencia real de orden de carga ENTRE las dos mitades
// (cada sección depende de cosas ya definidas antes por otros archivos —
// hookGlobal, DOM, S — nunca de una sección de la otra mitad), así que
// fusionarlas fue seguro. Las secciones de abajo conservan su numeración
// original de cada mitad (no se renumeraron a una sola secuencia continua,
// mismo criterio que sheet-behavior.js) — algunas ya son solo un comentario
// apuntando a dónde vive hoy esa funcionalidad, migrada en sesiones
// posteriores a la extracción original.
...
(function(){
  ...
  /* ====================================================
     3. INDICADOR DE SALUD FINANCIERA — migrado a js/modules/inicio.js
        ...
        Quedan como funciones globales; _renderMejoras() (más abajo en
        este mismo archivo) las sigue llamando desde acá porque ese mismo
        hook también dispara renderPresupuestos() (módulo de Análisis, no
        de Inicio).
  ==================================================== */
  ...
  /* ====================================================
     4. PROYECCIÓN FINANCIERA — migrado a js/modules/inicio.js
        ...
        (más abajo en este mismo archivo) la sigue llamando desde acá por
        el mismo motivo que la sección 3.
  ==================================================== */
...
/* ================================================================
   NAVEGACIÓN POR GESTOS — manejado por makeSwipeable(), en
   js/core/sheet-behavior.js (sección "SWIPE TO CLOSE" de ese archivo,
   no de este) — aclarado acá porque la numeración de secciones de
   arriba (1, 1b, 2, 3, 4, 6, 7, 8, 9, 10) es de este archivo, no la de
   sheet-behavior.js, y podía confundirse con su propia "sección 7".
   ================================================================ */
