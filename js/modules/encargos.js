...
/* ═══════════════════════════════════════════════════════════════
   ...
   (antes js/modules/encargos-personas.js — fusionado acá el 2026-08-03)

   Selector de persona en "Nuevo encargo", el hook que exige y asigna
   personaId al crear, y los botones/avatares de "ver perfil" en la
   lista y en el detalle de un encargo. Ver docs/encargos.md.
   ...
   PERSONA_COLORES/_inyectarPersonaSheets() (definidos en personas.js,
   que cargaba después). Esa premisa no se sostenía contra el código
   real: todas esas llamadas viven DENTRO de funciones (nunca a nivel
   superior del archivo), así que solo se ejecutan en el click —mucho
   después de que personas.js ya terminó de cargar, sin importar el
   orden entre <script defer>. Lo único que este bloque necesita a
   nivel superior (crearEncargo, renderEncargosList, abrirEncargoDetalle,
   openSheet) ya está definido arriba, en este mismo archivo. Fusión
   verificada con node --check; ver CHANGELOG.md#encargos para el detalle.
   ═══════════════════════════════════════════════════════════════ */
